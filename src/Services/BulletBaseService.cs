...
public class BulletBaseService
{
    ...
    {
        // 0. Update Users Table with new Goals
        ...
        // 11. NEW: Health Details
        await _db.Database.ExecuteSqlRawAsync(@"
            CREATE TABLE IF NOT EXISTS ""BulletHealthDetails"" (
                ""BulletItemId"" INTEGER NOT NULL PRIMARY KEY,
                ""WeightLbs"" DOUBLE PRECISION NOT NULL DEFAULT 0,
                ""CalculatedTDEE"" INTEGER NOT NULL DEFAULT 0,
                CONSTRAINT ""FK_BulletHealthDetails_BulletItems"" FOREIGN KEY (""BulletItemId"") REFERENCES ""BulletItems""(""Id"") ON DELETE CASCADE
            );
        ");

        // 12. NEW: Meals
        await _db.Database.ExecuteSqlRawAsync(@"
            CREATE TABLE IF NOT EXISTS ""BulletHealthMeals"" (
                ...
                CONSTRAINT ""FK_BulletHealthMeals_BulletItems"" FOREIGN KEY (""BulletItemId"") REFERENCES ""BulletItems""(""Id"") ON DELETE CASCADE
            );
        ");

        // 13. NEW: Workouts
        await _db.Database.ExecuteSqlRawAsync(@"
            CREATE TABLE IF NOT EXISTS ""BulletHealthWorkouts"" (
                ""Id"" SERIAL PRIMARY KEY,
                ""BulletItemId"" INTEGER NOT NULL,
                ""Name"" TEXT NOT NULL DEFAULT '',
                ""CaloriesBurned"" DOUBLE PRECISION NOT NULL DEFAULT 0,
                ""TimeSpentMinutes"" INTEGER NOT NULL DEFAULT 0,
                CONSTRAINT ""FK_BulletHealthWorkouts_BulletItems"" FOREIGN KEY (""BulletItemId"") REFERENCES ""BulletItems""(""Id"") ON DELETE CASCADE
            );
        ");
    }
    ...
    public async Task UpdateItemOrders(Dictionary<int, int> updates) { if (!updates.Any()) return; var ids = updates.Keys.ToList(); var items = await _db.BulletItems.Where(x => ids.Contains(x.Id)).ToListAsync(); foreach(var item in items) { if(updates.TryGetValue(item.Id, out int newOrder)) item.SortOrder = newOrder; } await _db.SaveChangesAsync(); }
