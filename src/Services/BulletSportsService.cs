            ...
            foreach (var el in items.EnumerateArray())
            {
                ...
                if (type == "sports")
                {
                    ...
                    if (!string.IsNullOrEmpty(title) && title.Contains("@"))
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            string awayAbbr = parts[0].Trim();
                            string homeAbbr = parts[1].Trim();
                    ...
                    // 5. Create Detail
                    var detail = new BulletGameDetail {
                        BulletItemId = item.Id,
                        LeagueId = leagueId,
                        SeasonId = seasonId,
                        HomeTeamId = homeId,
                        AwayTeamId = awayId,
                        IsComplete = (el.TryGetProperty("status", out var st) && st.ToString() == "completed"),
