...
public class BulletBirthdayService
{
    private readonly AppDbContext _db;

    public BulletBirthdayService(AppDbContext db)
    {
        _db = db;
    }

    public class BirthdayDTO : BulletItem
    {
        public BulletBirthdayDetail Detail { get; set; } = new();
        public List<BulletItemNote> Notes { get; set; } = new();
    }

    public async Task<List<BirthdayDTO>> GetBirthdaysForRange(int userId, DateTime start, DateTime end)
    {
        var items = await (from baseItem in _db.BulletItems
                           join detail in _db.BulletBirthdayDetails on baseItem.Id equals detail.BulletItemId
                           where baseItem.UserId == userId 
                                 && baseItem.Date >= start && baseItem.Date <= end
                                 && baseItem.Type == "birthday"
    ...
    public async Task SaveBirthday(BirthdayDTO dto)
    {
        ...
        if (dto.Id > 0) item = await _db.BulletItems.FindAsync(dto.Id);
            ...
            item = new BulletItem { UserId = dto.UserId, Type = "birthday", CreatedAt = DateTime.UtcNow };
        ...
        var detail = await _db.BulletBirthdayDetails.FindAsync(item.Id);
        if (detail == null) { detail = new BulletBirthdayDetail { BulletItemId = item.Id }; await _db.BulletBirthdayDetails.AddAsync(detail); }

        detail.DOB_Year = dto.Detail.DOB_Year;
    ...
    public async Task<int> ImportFromOldJson(int userId, string jsonContent)
    {
        ...
        if(items.ValueKind == JsonValueKind.Array)
        {
            foreach (var el in items.EnumerateArray())
            {
                ...
                if (GetStr("type").ToLower() == "birthday")
                {
                    ...
                    var item = new BulletItem { 
                        UserId = userId, Type = "birthday", CreatedAt = DateTime.UtcNow, Date = itemDate,
                    ...
                    var detail = new BulletBirthdayDetail { BulletItemId = item.Id };
                    
                    // Attempt to extract year if available in JSON, otherwise 0
                    if (el.TryGetProperty("year", out var yr)) detail.DOB_Year = yr.GetInt32();
                    else if (el.TryGetProperty("dob_year", out var dyr)) detail.DOB_Year = dyr.GetInt32();
                    
                    await _db.BulletBirthdayDetails.AddAsync(detail);
