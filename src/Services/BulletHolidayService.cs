...
public class BulletHolidayService
{
    private readonly AppDbContext _db;

    public BulletHolidayService(AppDbContext db)
    {
        _db = db;
    }

    public class HolidayDTO : BulletItem
    {
        public BulletHolidayDetail Detail { get; set; } = new();
        public List<BulletItemNote> Notes { get; set; } = new();
    }

    public async Task<List<HolidayDTO>> GetHolidaysForRange(int userId, DateTime start, DateTime end)
    {
        var items = await (from baseItem in _db.BulletItems
                           join detail in _db.BulletHolidayDetails on baseItem.Id equals detail.BulletItemId
                           where baseItem.UserId == userId 
                                 && baseItem.Date >= start && baseItem.Date <= end
                                 && baseItem.Type == "holiday"
    ...
    public async Task SaveHoliday(HolidayDTO dto)
    {
        ...
        if (dto.Id > 0) item = await _db.BulletItems.FindAsync(dto.Id);
            ...
            item = new BulletItem { UserId = dto.UserId, Type = "holiday", CreatedAt = DateTime.UtcNow };
        ...
        var detail = await _db.BulletHolidayDetails.FindAsync(item.Id);
        if (detail == null) { detail = new BulletHolidayDetail { BulletItemId = item.Id }; await _db.BulletHolidayDetails.AddAsync(detail); }

        detail.IsWorkHoliday = dto.Detail.IsWorkHoliday;
    ...
    public async Task<int> ImportFromOldJson(int userId, string jsonContent)
    {
        ...
        if(items.ValueKind == JsonValueKind.Array)
        {
            foreach (var el in items.EnumerateArray())
            {
                ...
                if (GetStr("type").ToLower() == "holiday")
                {
                    ...
                    var item = new BulletItem { 
                        UserId = userId, Type = "holiday", CreatedAt = DateTime.UtcNow, Date = itemDate,
                    ...
                    // Default to work if missing
                    if(string.IsNullOrEmpty(item.Category)) item.Category = "work";

                    await _db.BulletItems.AddAsync(item);
                    await _db.SaveChangesAsync(); 

                    var detail = new BulletHolidayDetail { BulletItemId = item.Id };
                    
                    if (el.TryGetProperty("isWorkHoliday", out var wh)) detail.IsWorkHoliday = wh.GetBoolean();
                    
                    await _db.BulletHolidayDetails.AddAsync(detail);
