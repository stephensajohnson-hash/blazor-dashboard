...
public class BulletAnniversaryService
{
    private readonly AppDbContext _db;

    public BulletAnniversaryService(AppDbContext db)
    {
        _db = db;
    }

    public class AnniversaryDTO : BulletItem
    {
        public BulletAnniversaryDetail Detail { get; set; } = new();
        public List<BulletItemNote> Notes { get; set; } = new();
    }

    public async Task<List<AnniversaryDTO>> GetAnniversariesForRange(int userId, DateTime start, DateTime end)
    {
        var items = await (from baseItem in _db.BulletItems
                           join detail in _db.BulletAnniversaryDetails on baseItem.Id equals detail.BulletItemId
                           where baseItem.UserId == userId 
                                 && baseItem.Date >= start && baseItem.Date <= end
                                 && baseItem.Type == "anniversary"
                           select new AnniversaryDTO 
                           { 
    ...
    public async Task SaveAnniversary(AnniversaryDTO dto)
    {
        ...
        if (dto.Id > 0) item = await _db.BulletItems.FindAsync(dto.Id);
            ...
            item = new BulletItem { UserId = dto.UserId, Type = "anniversary", CreatedAt = DateTime.UtcNow };
        ...
        var detail = await _db.BulletAnniversaryDetails.FindAsync(item.Id);
        if (detail == null) { detail = new BulletAnniversaryDetail { BulletItemId = item.Id }; await _db.BulletAnniversaryDetails.AddAsync(detail); }

        detail.AnniversaryType = dto.Detail.AnniversaryType;
        detail.FirstYear = dto.Detail.FirstYear;
    ...
    public async Task<int> ImportFromOldJson(int userId, string jsonContent)
    {
        ...
        if(items.ValueKind == JsonValueKind.Array)
        {
            foreach (var el in items.EnumerateArray())
            {
                ...
                if (GetStr("type").ToLower() == "anniversary")
                {
                    ...
                    var item = new BulletItem { 
                        UserId = userId, Type = "anniversary", CreatedAt = DateTime.UtcNow, Date = itemDate,
                    ...
                    var detail = new BulletAnniversaryDetail { BulletItemId = item.Id };
                    
                    if (el.TryGetProperty("anniversaryType", out var at)) detail.AnniversaryType = at.GetString() ?? "Other";
                    if (el.TryGetProperty("firstYear", out var fy)) detail.FirstYear = fy.GetInt32();
                    
                    await _db.BulletAnniversaryDetails.AddAsync(detail);
