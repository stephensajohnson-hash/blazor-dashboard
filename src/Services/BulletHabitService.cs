...
public class BulletHabitService
{
    ...
    public BulletHabitService(IDbContextFactory<AppDbContext> factory)
    {
        _factory = factory;
    }
    ...
    public async Task<List<HabitDTO>> GetHabits(int userId)
    {
        ...
        var items = await (from baseItem in db.BulletItems
                           join detail in db.BulletHabitDetails on baseItem.Id equals detail.BulletItemId
                           where baseItem.UserId == userId && baseItem.Type == "habit"
                           select new HabitDTO 
                           { 
    ...
    public async Task SaveHabit(HabitDTO dto)
    {
        ...
        var detail = await db.BulletHabitDetails.FindAsync(item.Id);
        if (detail == null) { detail = new BulletHabitDetail { BulletItemId = item.Id }; await db.BulletHabitDetails.AddAsync(detail); }
        ...
        if(items.ValueKind == JsonValueKind.Array)
        {
            foreach (var el in items.EnumerateArray())
            {
                ...
                if (type == "habit")
                {
                    ...
                    var item = new BulletItem { UserId = userId, Type = "habit", CreatedAt = DateTime.UtcNow, Date = date, Title = (el.TryGetProperty("title", out var tit) ? tit.ToString() : ""), OriginalStringId = (el.TryGetProperty("id", out var oid) ? oid.ToString() : "") };
                    await db.BulletItems.AddAsync(item);
                    await db.SaveChangesAsync();

                    var detail = new BulletHabitDetail { BulletItemId = item.Id };
                    if(el.TryGetProperty("streak", out var st)) detail.StreakCount = st.GetInt32();
                    await db.BulletHabitDetails.AddAsync(detail);
                    count++;
                }
            }
            await db.SaveChangesAsync();
